...
export default function ShowPointsUser(){
    ...
    useEffect(() => {
            ...
            debugger
            axios.get("https://localhost:7192/api/PointsDetails/0," + userId)
                .then((response) => {
                    if (response.data != '')
                    {
            ...
            {list && list[0] && list[0].userPointId &&
              <table>
                ...
                <tr key={index}>
                   ...
                   <td>{item.userPoint.couponAmount}</td>
                   <td>{item.userPoint.dueDate.split('T')[0]} , {item.userPoint.dueDate.split('T')[1]}</td>
